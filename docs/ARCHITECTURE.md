...
> **Status: partially implemented.** This is the target design agreed during the architecture planning phase (see `docs/DECISIONS.md` ADR-002, ADR-003, ADR-004). Domain, Persistence, and the core Sleeper sync module (Phase 2 — Sleeper Experience) are built; see `PROJECT_CONTEXT.md` for exact current state. Everything else described below (API/frontend for Phase 2, all of Phase 3 and Phase 4) remains the target, not yet built.
...
- **Player** — global, first-class entity (canonical ID, name, position, team), synced independently of any league. Bootstrapped from Sleeper's player list in Phase 2 (Sleeper Experience) so `Roster` has something to reference; enriched with additional sources starting Phase 3 (Unified Player Profiles). Never duplicated onto league-scoped rows — see ADR-003.
- **Roster / Matchup** — synced from Sleeper per League, current season only for Phase 2. `Roster` references `Player` by FK rather than duplicating player metadata. `Matchup` has no home/away concept — each side is a `MatchupParticipant` row (see ADR-007). Standings, power rankings, and trends computed from this data are Phase 4 (Analytics) scope, not Phase 2 — see ADR-006 and ADR-009. A persisted `StandingSnapshot` is deferred to Phase 4 as well, once true multi-season history makes recomputing from raw matchup history on every read impractical.
