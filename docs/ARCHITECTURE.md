...
- **Player** — global, first-class entity (canonical ID, name, position, team), synced independently of any league. Bootstrapped from Sleeper's player list in Phase 2 (Sleeper Platform) so `Roster` has something to reference; enriched with additional sources starting Phase 3 (Unified Player Profiles). Never duplicated onto league-scoped rows — see ADR-003.
