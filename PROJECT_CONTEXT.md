...
- **Milestone 4 (API — `LeaguesController`)** — partially done. Ships `GET /api/leagues/{sleeperLeagueId}/preview`, `POST /api/leagues/{sleeperLeagueId}/sync`, and raw league reads (Milestone 4a): `GET /api/leagues/{sleeperLeagueId}` (detail), `GET .../rosters` (every team's current-season roster, player identity embedded), `GET .../matchups` (every persisted matchup, real scores) — all via a new `ILeagueReadService`, bulk-only (no single-team/single-week variants yet). Enum fields (`Position`, `Slot`) now serialize as readable strings, not numbers, via a global `JsonStringEnumConverter` added in `Program.cs`. Milestone 4b (players — single + search) not yet started.
...
**Next:** the remaining Phase 2 (Sleeper Experience) gap is Milestone 4b — raw read endpoints for players (single + search) — see `docs/ROADMAP.md` for full phase detail.
